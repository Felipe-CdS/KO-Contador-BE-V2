...
class AdminCreateNewTransactionService {
	...
	async execute({ username, comission_entries, number_identifier, transaction_date }){
		...
		const checkThisMonth					= await this.checkOtherTransactionThisMonth(new Date(transaction_date), user.user_id, number_identifier, transactionRepository, taxTableRepository);
		...
		const rbt_12							= await this.getRBT12(user.user_id, thisMonthTotal, transactionRepository);
...
export { AdminCreateNewTransactionService }